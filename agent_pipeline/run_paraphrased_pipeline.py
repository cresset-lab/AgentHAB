...
DATASET_PATH = Path("datasets") / "paraphrased_dataset.json"
RESULTS_DIR = (
    Path("generated_rules") / "results" / "agents_workflow" / "paraphrased"
)
...
def fetch_system_context() -> SystemContext | None:
    ...
    print("\n=== Fetching live system context via MCP server (agent pipeline, paraphrased) ===")
    try:
...
def load_dataset(path: Path) -> List[Dict[str, Any]]:
    """Load the paraphrased dataset (id, text) pairs."""
...
def run_agent_pipeline_on_paraphrased_dataset(only_id: int | None = None) -> None:
    """
    Run the full agent pipeline on the paraphrased dataset.

    For each example in datasets/paraphrased_dataset.json (or a single example
    if `only_id` is provided), this will:
      - run the generation/validation loop (with optional context validation),
      - write the final openHAB DSL code to results/agents_workflow/paraphrased/<id>.rules,
      - store a JSON summary of all runs in results/agents_workflow/paraphrased/summary.json.
    """
    print(f"Loading paraphrased dataset from {DATASET_PATH} ...")
    ...
    for example in examples:
        ...
        print(f"\n=== Agent pipeline run (paraphrased) for id={ex_id} ===")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "paraphrased dataset (or a single example by id)."
        )
    )
    parser.add_argument(
        "--id",
...
def main() -> None:
    args = parse_args()
    run_agent_pipeline_on_paraphrased_dataset(only_id=args.only_id)
